...
def test_query_polygon_news(mock_openai, client):
    ...
    response = client.post(
        "/query", data={"input_text": "What is the latest news on AAPL?"}
    )
    assert response.status_code == 200
...
def test_query_polygon_details(mock_openai, client):
    ...
    mock_response.chat.completions.create.return_value = Mock(
        choices=[
            Mock(message=Mock(content="[PolygonTickerDetails, {'ticker': 'AAPL'}]"))
        ]
    )
    mock_openai.return_value = mock_response
...
def test_query_polygon_aggregates(mock_openai, client):
    ...
    mock_response.chat.completions.create.return_value = Mock(
        choices=[
            Mock(
                message=Mock(
                    content="[PolygonAggregates, {'ticker': 'AAPL', 'timespan': 'day', 'timespan_multiplier': 1, 'from_date': '2024-03-07', 'to_date': '2024-03-14'}]"
                )
            )
        ]
    )
    ...
    response = client.post(
        "/query",
        data={
            "input_text": "What has been Apple's daily closing price between March 7, 2024, and March 14, 2024?"
        },
    )
    assert response.status_code == 200
    assert "output" in response.json

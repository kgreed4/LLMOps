...
def determine_tool(input):
    ...
    api_input = completion.choices[0].message.content.split("[")[1].split("]")[0].split(",")
    print(api_input)
...
def call_API(input):
    ...
    if input[0] == "PolygonAggregates":
        api_ans = client.list_aggs(input[1]['ticker'], input[1]['timespan_multiplier'], input[1]['timespan'], input[1]['from_date'], input[1]['to_date'])
        ans = []
        for i in api_ans:
            ans.append(i)
        return ans
...
def get_insights(questionType, api_output, question):
    ...
    aggregate_instructions = base_instructions + """ The data is stock prices over a specific time period. 
    ...
    *Open = opening price, High = highest price, Low = lowest price, Close = closing price, Volume = number of shares traded, VWAP = volume-weighted average price, Timestamp = Unix timestamp, Transactions = number of transactions, OTC = over-the-counter trades
    ...
    """ 
    news_instructions = base_instructions + """ The data is a list of news articles for a specific ticker symbol.
    EXAMPLE 2:
    ...
    """
    ticker_details_instructions = base_instructions + """ 
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": instructions},
            {"role": "user", "content": f"Answer this {question}, using this information, {api_output}."}
        ]
    )
...
def query():
    ...
    try:
        ...
        return jsonify({"output": f"An error occurred with the PolygonAPI. Please try again in a few minutes: {e}"})
